...
<!--/Pre Loader -->
    ...
    <!-- 本表的查询条件 -->
        ...
        function setDataRow(item, number) {
            ...
            if(window.sessionStorage.getItem('role') == '会员'){
                //兑换按钮
                var duihuanBtn = document.createElement('button');
                var duihuanAttr = 'duihuan(' + item.id + ')';
                duihuanBtn.setAttribute("type", "button");
                duihuanBtn.setAttribute("class", "btn btn-warning btn-sm");
                duihuanBtn.setAttribute("onclick", duihuanAttr);
                duihuanBtn.innerHTML = "兑换";
                btnGroup.appendChild(duihuanBtn);
            }
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("jifengoods/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addjifengoods", "addjifengoods");
            window.location.href = "add-or-update.jsp"
        }
