...
public class GoodsController {
    ...
    //级联表service
    @Autowired
    private YonghuService yonghuService;//销售
    @Autowired
    ...
    */
    ...
    public R save(@RequestBody GoodsEntity goods, HttpServletRequest request){
        ...
        Wrapper<GoodsEntity> queryWrapper = new EntityWrapper<GoodsEntity>()
            ...
            .eq("shangjia_types", goods.getShangjiaTypes())
        ...
        if(goodsEntity==null){
            ...
            if("用户".equals(role)){
                YonghuEntity yonghuEntity = yonghuService.selectById(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
                goods.setShangjiaTypes(yonghuEntity.getShangjiaTypes());
            }else if("商家管理员".equals(role) ){
                ShangjiaguanliEntity shangjiaguanli = shangjiaguanliService.selectById(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
                goods.setShangjiaTypes(shangjiaguanli.getShangjiaTypes());
            }
            goods.setFlag(1);
