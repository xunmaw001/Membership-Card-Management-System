package com.entity.view;

import com.entity.JifengoodsEntity;

import com.baomidou.mybatisplus.annotations.TableName;
import org.apache.commons.beanutils.BeanUtils;
import java.lang.reflect.InvocationTargetException;
import org.springframework.format.annotation.DateTimeFormat;
import com.fasterxml.jackson.annotation.JsonFormat;

import java.io.Serializable;
import java.util.Date;

/**
 * 积分商品
 * 后端返回视图实体辅助类
 * （通常后端关联的表或者自定义的字段需要返回使用）
 * @author 
 * @email
 * @date 2021-04-21
 */
@TableName("jifengoods")
public class JifengoodsView extends JifengoodsEntity implements Serializable {
    private static final long serialVersionUID = 1L;
		/**
		* 商品种类的值
		*/
		private String goodsValue;
		/**
		* 商家的值
		*/
		private String shangjiaValue;



	public JifengoodsView() {

	}

	public JifengoodsView(JifengoodsEntity jifengoodsEntity) {
		try {
			BeanUtils.copyProperties(this, jifengoodsEntity);
		} catch (IllegalAccessException | InvocationTargetException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}



			/**
			* 获取： 商品种类的值
			*/
			public String getGoodsValue() {
				return goodsValue;
			}
			/**
			* 设置： 商品种类的值
			*/
			public void setGoodsValue(String goodsValue) {
				this.goodsValue = goodsValue;
			}
			/**
			* 获取： 商家的值
			*/
			public String getShangjiaValue() {
				return shangjiaValue;
			}
			/**
			* 设置： 商家的值
			*/
			public void setShangjiaValue(String shangjiaValue) {
				this.shangjiaValue = shangjiaValue;
			}













}
